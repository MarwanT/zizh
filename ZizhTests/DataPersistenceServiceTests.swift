...
final class SwiftDataServiceTests {
  ...
  func addItemSuccessfully() async throws {
    // given
    let recordings = MockData.recordingsData(count: 1).map { record in
      Recording.entityFrom(record) as Recording
    }
    // when
  ...
  func removeExistingItem() async throws {
    // given
    let recordings = MockData.recordingsData(count: 2).map { record in
      Recording.entityFrom(record) as Recording
    }
  ...
  func fetchAllItemsSuccessfully() async throws {
    // given
    let recordings = MockData.recordingsData(count: 3).map { record in
      Recording.entityFrom(record) as Recording
    }
    for recording in recordings {
      try await sut.add(item: recording).values.first()
